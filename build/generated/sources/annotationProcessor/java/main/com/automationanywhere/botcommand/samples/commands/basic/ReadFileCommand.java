...
public final class ReadFileCommand implements BotCommand {
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    if(convertedParameters.containsKey("file")) {
      ...
      if (lastIndxDot == -1 || lastIndxDot >= filePath.length()) {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.FileExtension","file","txt"));
      }
      String fileExtension = filePath.substring(lastIndxDot + 1);
      if(!Arrays.stream("txt".split(",")).anyMatch(fileExtension::equalsIgnoreCase))  {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.FileExtension","file","txt"));
      }

    }
    try {
      Optional<Value> result =  Optional.ofNullable(command.action((String)convertedParameters.get("file")));
